...
|-------|--------------------------|----------------------------------------------------------------------------------|-------------|-----------------|
| AC-1  | Event-driven             | tests/test_avv_kaskade.py::test_ac1_register_processor_happy_path                | green       | Phase 5 (green) |
| AC-1.1| Unwanted-behavior        | tests/test_avv_kaskade.py::test_ac11_register_processor_expired_avv_rejected     | green       | Phase 5 (green) |
| AC-2  | State-driven             | tests/test_avv_kaskade.py::test_ac2_geeignetheitspruefung_report_complete         | green       | Phase 5 (green) |
| AC-2.1| Unwanted-behavior        | tests/test_avv_kaskade.py::test_ac21_vendor_non_coop_flagged_in_report            | green       | Phase 5 (green) |
| AC-3  | Event-driven             | tests/test_avv_kaskade.py::test_ac3_attestor_dsk_kp13_packet_shape                | green       | Phase 5 (green) |
| AC-4  | Event-driven (deferred)  | tests/test_avv_kaskade.py::test_ac4_export_audit_packet_3state                    | green | Phase 5 (green) — **deferred to BFSG-AA reviewer pool** per STALE_PENDING_FLAG |
| AC-5  | Event-driven             | tests/test_avv_kaskade.py::test_ac5_nis2_bsig_evidence_locker                     | green       | Phase 5 (green) |
...
## §9 Phase 5 (tactical) refactor SHIPPED status

- [x] `repo/src/kurort_engine/avv_kaskade/_validation.py` (DRY helpers: `compute_avv_hash`, `compute_canonical_sha256`, `assert_date_constraint` — 15 LOC, ≤20 envelope)
- [x] Re-imports in 5 src files: `processor.py`, `attestor.py`, `geeignetheitspruefung.py`, `reporter.py`, `tom_evidence.py` (no behavioral change)
- [x] `repo/docs/avv_kaskade/README.md` (174 lines, 7 sections: Overview + AC-1..AC-5 quickstart + DSK-KP13 + NIS2-§38-BSIG + 6-processor chain + AC-4 deferred_review + CLI usage)
- [x] `kurort avv` CLI dispatcher: 3 subcommands (`attest`, `geeignetheitspruefung`, `version`) wired in `repo/src/kurort_engine/__init__.py` + `repo/src/kurort_engine/__main__.py`
- [x] `repo/tests/test_avv_kaskade_chain.py` (6-processor Bad Orb Kurverwaltung chain integration test — 1 test, uses `_reset_registry()` for test isolation)
- [x] Full Phase 5 verification: `PYTHONPATH=src python -m pytest tests/test_avv_kaskade.py tests/test_avv_kaskade_chain.py` → **8 passed, 0 failed, 0 error, 0 skipped**
- [x] `ruff check src/kurort_engine/avv_kaskade/` → exit 0 (all checks passed)
- [x] `shasum -a 256 spec/avv_kaskade/spec.yaml` → `d9e6c5520ca8bec03a179a232bf20333653be976aaf403ce8d776dbc03f28666` (UNCHANGED from Phase 1)
- [x] 6 SHA anti-drift files: `git diff HEAD~N..HEAD -- <6 SHA files>` = 0 lines per file (Pattern C chain-extension preserved)
- [x] AC status (NOT a status flip — all 7 ACs stay `green`): AC-1, AC-1.1, AC-2, AC-2.1, AC-3, AC-4, AC-5 all at `green` per `spec.lock.md` traceability matrix

**Phase 5 (tactical) refactor SHIPPED.** Cycle continues to Phase 6 (strategic verify) per next_phase_todos.
