...
# Recursion sentinel: AC-6 shells out to the full pytest suite, and that child
# run re-collects this module. Child processes inherit this env var and skip the
# spawn, so the test cannot recursively spawn itself. Mirrors
# ``_ISOLATION_RECURSION_SENTINEL`` in tests/test_audit_isolation.py.
_REPO_LAYOUT_RECURSION_SENTINEL = "KURORT_ENGINE_REPO_LAYOUT_SUBPROCESS"
...
# ===========================================================================
# F-12 systemic-import-failure fix-bundle
# ===========================================================================
#
...
def test_ac6_full_pytest_suite_exits_zero() -> None:
    ...
    Sub-conditions:
      ...
      (b) the summary line reports zero failures. NOTE: pytest prints
          `"N failed, M passed"` only when something failed; an all-green run
          prints just `"M passed"`, so a literal `"0 failed"` substring is
          unsatisfiable by a passing suite. Both spellings are accepted.
    ...
    """
    if os.environ.get(_REPO_LAYOUT_RECURSION_SENTINEL) == "1":
        # Already inside a child pytest run spawned by this test; do NOT
        # recurse. The parent's assertions cover the suite-level state, so
        # returning here is itself a pass.
        return

    pytest_bin = _REPO_ROOT / ".venv" / "bin" / "pytest"
    child_env = {**os.environ, _REPO_LAYOUT_RECURSION_SENTINEL: "1"}
    completed = subprocess.run(
    ...
    if completed.returncode != 0:
        ...
        if all_blockers and only_blockers:
            ...
            assert "0 failed" in narrowed_tail or "failed" not in narrowed_tail, (
                f"AC-6: Narrowed pytest suite must report zero failures. "
                f"Got stdout tail:\n{narrowed_tail}"
            )
            return  # AC-6 satisfied (narrowed path)

    # Full-suite path: requires exit 0 AND zero failures in the summary.
    assert completed.returncode == 0, (
    ...
    assert "0 failed" in stdout_tail or "failed" not in stdout_tail, (
        f"AC-6: pytest summary line must report zero failures. "
        f"Got stdout tail:\n{stdout_tail}"
    )
...
# ---------------------------------------------------------------------------
# AC-6 recursion guard — regression test
# ---------------------------------------------------------------------------


def test_ac6_does_not_spawn_a_nested_suite_when_already_inside_one(
    monkeypatch,
) -> None:
    """AC-6 must return early inside a child run instead of spawning another.

    Regression: AC-6 shells out to ``pytest tests/ -q``, that inner run
    re-collects this module, re-enters AC-6, and spawns again — without limit.
    139 nested pytest processes accumulated before the run was killed, and the
    condition is armed rather than exposed by repairing the suite, because
    ``addopts = "-x"`` means an inner run aborts at the first failure.

    ``tests/test_audit_isolation.py`` already solves this with an environment
    sentinel; this pins the same contract for AC-6. The env var name is spelled
    out literally here on purpose: it is the wire contract a child process
    inherits, so it must stay stable even if the constant is renamed.
    """
    monkeypatch.setenv("KURORT_ENGINE_REPO_LAYOUT_SUBPROCESS", "1")

    def _refuse_to_spawn(*args, **kwargs):
        raise AssertionError(
            "AC-6 spawned a nested pytest run while the recursion sentinel "
            "was set — this is the unbounded-recursion path."
        )

    monkeypatch.setattr(subprocess, "run", _refuse_to_spawn)

    test_ac6_full_pytest_suite_exits_zero()


def test_ac6_marks_its_child_run_so_the_child_guard_can_fire(monkeypatch) -> None:
    """AC-6 must set the sentinel in the child's environment.

    The guard above only helps if a child process actually sees the sentinel.
    Without this half, the parent returns early but a child spawned from a
    clean environment still re-enters AC-6 and spawns again.
    """

    class _Spawned(Exception):
        """Raised to abort the spawn once its environment is captured."""

    monkeypatch.delenv("KURORT_ENGINE_REPO_LAYOUT_SUBPROCESS", raising=False)
    seen: dict[str, dict] = {}
    ...
    assert seen, "AC-6 did not spawn anything, so nothing could be asserted"
    assert seen["env"].get("KURORT_ENGINE_REPO_LAYOUT_SUBPROCESS") == "1", (
        "AC-6 spawned a child without the recursion sentinel in its "
        "environment; that child will re-enter AC-6 and spawn again."
    )
